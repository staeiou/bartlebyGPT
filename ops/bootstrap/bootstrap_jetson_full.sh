...
WHEEL_CACHE_DIR="${VLLM_WHEEL_CACHE_DIR:-/tmp/bartleby-wheels}"
...
resolve_vllm_wheel_path() {
  ...
  if [[ -n "${WHEEL_URL}" ]]; then
    if ! command -v curl >/dev/null 2>&1; then
      echo "ERROR: curl is required to download VLLM_WHEEL_URL." >&2
      return 1
    fi
    local wheel_name
    ...
    wheel_path="${WHEEL_CACHE_DIR}/${wheel_name}"
    mkdir -p "${WHEEL_CACHE_DIR}"
    echo "  Downloading vLLM wheel from VLLM_WHEEL_URL -> ${wheel_path}"
...
if ! WHEEL_PATH="$(resolve_vllm_wheel_path)"; then
  echo "ERROR: vLLM wheel not found." >&2
