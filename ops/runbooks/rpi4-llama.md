...
## Solix BLE Power Monitor

Each Pi runs `solix-monitor.service` alongside the inference stack. It reads power telemetry from the Anker Solix C300X DC battery over BLE and feeds `power_telemetry.py` via the ESPHome-compat `/sensor/power` shim.
...
See `ops/runbooks/solix-ble.md` for full setup, MAC addresses, firmware detection, and troubleshooting.
